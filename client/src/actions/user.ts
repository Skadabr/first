import { StatusKinds, CLEAN_STATE } from "../constants";
...
export function userAdd(name, email, token, rate, status: StatusKinds = StatusKinds.PEACE ) {
  return {
    type: USER_ADD,
...
export function userUpdateStatus(status: StatusKinds) {
  return {
    type: USER_UPDATE,
    payload: { status }
  };
