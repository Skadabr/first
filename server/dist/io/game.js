...
exports.default = function(ws, { models, logger }) {
  ...
  ws.on("disconnect", async () => {
    const user = await User.findOneAndUpdate(
      { socket_id: ws.id },
      { socket_id: null, status: _user.PEACE, game: null }
    );
  ...
  ws.on(USER_READY, async () => {
    ...
    ws.emit(OPPONENT_UPSERT, (0, _only2.default)(opponent, "name status"));
    ws.broadcast.emit(
      OPPONENT_UPSERT,
      (0, _only2.default)(opponent, "name status")
    );
    ws.broadcast.emit(
      OPPONENT_UPSERT,
      (0, _only2.default)(ws.user, "name status")
    );
